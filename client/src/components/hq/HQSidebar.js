...
function HQSidebar({ onReport }) {
  return (
    <aside className="fixed left-0 top-0 z-50 flex h-full w-60 flex-col border-r border-slate-800/10 bg-[#10131a]">
      <div className="p-6">
      ...
      <nav className="flex-1 space-y-1 px-2">
        <NavLink exact to="/" className={navClass} activeClassName={activeClass}>
          <span className="material-symbols-outlined text-[20px]">map</span>
          Map
        </NavLink>
        <NavLink to="/raids" className={navClass} activeClassName={activeClass}>
          <span className="material-symbols-outlined text-[20px]">radar</span>
          Raids
        </NavLink>
        <NavLink to="/routes" className={navClass} activeClassName={activeClass}>
      ...
      <div className="mt-auto p-4">
        ...
        <div className="mt-6 flex items-center gap-3 border-t border-outline-variant/10 px-2 py-4">
          <div className="flex h-10 w-10 items-center justify-center rounded-full bg-surface-container-high text-[10px] font-bold text-primary">
            PF
          </div>
